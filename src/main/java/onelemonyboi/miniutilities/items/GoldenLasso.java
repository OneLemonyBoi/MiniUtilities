...
public class GoldenLasso extends Item {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        if (context.getWorld() instanceof ServerWorld && context.getItem().getTag() != null && context.getItem().getTag().contains("EntityTag")) {
            EntityType.byKey(context.getItem().getChildTag("EntityTag").getString("id")).get().spawn((ServerWorld) context.getWorld(), context.getItem().getTag(), null, null, context.getPos().offset(context.getFace()), SpawnReason.MOB_SUMMONED, false, false);
